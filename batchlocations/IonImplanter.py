...
class IonImplanter(QtCore.QObject):
    ...
    def __init__(self, _env, _output=None, _params = {}):
        ...
        self.params = {}
        self.params['specification'] = self.tr("IonImplanter consists of:\n")
        self.params['specification'] += self.tr("- Input container\n")
        self.params['specification'] += self.tr("- A number of loadlocks\n")
        self.params['specification'] += self.tr("- A number of process lanes\n")
        self.params['specification'] += self.tr("- Output container\n")
        self.params['specification'] += "\n"
        self.params['specification'] += self.tr("Cassettes are loaded into the loadlocks, which are ")
        self.params['specification'] += self.tr("then held for a set time (for evacuation). Subsequently, the ")
        self.params['specification'] += self.tr("wafers are processed on belts and enter into buffer cassettes. ")
        self.params['specification'] += self.tr("When the buffer cassette is full, the wafers return on the same belt ")
        self.params['specification'] += self.tr("to the loadlock. After a set time (for repressurization) the cassettes ")
        self.params['specification'] += self.tr("are placed in the output buffer.\n")
        self.params['specification'] += "\n"
        self.params['specification'] += self.tr("There is a downtime procedure defined for the whole tool, during which ")         
        self.params['specification'] += self.tr("maintenance is performed.")          
