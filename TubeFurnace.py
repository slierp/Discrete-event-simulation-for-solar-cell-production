...
class TubeFurnace(object):
    ...
    def __init__(self, env, name="", process_batch_size=500, process_time=60*60, cool_time=10*60,
        ...
        print str(self.env.now) + " - [TubeFurnace][" + self.name + "] Added a tube furnace"        
    ...
    def report(self):
        print "[TubeFurnace][" + self.name + "] Units processed: " + str(self.transport_counter - self.output.container.level)
        for i in self.batchprocesses:
            print "[TubeFurnace][" + self.name + "][" + self.batchprocesses[i].name + "] Idle time: " + str(np.round(self.batchprocesses[i].idle_time(),1)) + " %" 
        ...
        while True:
            ...
            for i in self.batchprocesses:
                ...
                if (self.batchprocesses[i].container.level == 0) & \
                    ...
                    #print str(self.env.now) + " - [TubeFurnace][" + self.name + "] Started a process"
    ...
    def run_load_in(self):
        while True:
            ...
            #print str(self.env.now) + " - [TubeFurnace][" + self.name + "] Loading trigger received"
            ...
            #print str(self.env.now) + " - [TubeFurnace][" + self.name + "] Loaded batch"

    def run_load_out(self):
        while True:
            yield self.load_out_start
            #print str(self.env.now) + " - [TubeFurnace][" + self.name + "] Unloading trigger received"
            for i in np.arange(self.process_batch_size/self.cassette_size): # how to ensure it is an integer?
                yield self.env.timeout(30) # very critical for throughput
            ...
            yield self.load_in_out_end.succeed()
            self.load_in_out_end = self.env.event() # make new event            
            #print str(self.env.now) + " - [TubeFurnace][" + self.name + "] Unloaded batch"          